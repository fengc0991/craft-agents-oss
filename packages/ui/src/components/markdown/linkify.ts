...
const FILE_PATH_TOKEN_PATTERN = `[^\\s()[\\]{}<>]+`
const FILE_PATH_REGEX_SOURCE = `(?:^|[\\s([\\{<])((?:/|~/|\\./|\\.\\./|${FILE_PATH_TOKEN_PATTERN}/)?${FILE_PATH_TOKEN_PATTERN}\\.(?:${FILE_EXTENSIONS_PATTERN}))(?=[\\s)\\]}\\.,:;!?>]|$)`
...
const FILE_PATH_TARGET_REGEX = new RegExp(
  `^(?![a-z][a-z0-9+.-]*:)(?:/|~/|\\./|\\.\\./|${FILE_PATH_TOKEN_PATTERN}/)?${FILE_PATH_TOKEN_PATTERN}\\.(?:${FILE_EXTENSIONS_PATTERN})$`,
  'iu'
)
